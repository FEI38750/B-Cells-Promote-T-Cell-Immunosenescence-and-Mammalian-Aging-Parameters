...
setwd("~/downloads")
TB_cell.combined <- readRDS("/Users/feiwu/Downloads/TB_cell.combined.RDS")

DefaultAssay(TB_cell.combined) <- "SCT"
#subset(TB_cell.combined, features = c("Gpr151","Lvrn"))
...
Idents(TB_cell.combined) <- "orig.ident"
#DefaultAssay(TB_cell.combined) <- "SCT"

# 1. Define the niches/microenvironments of interest
niches = list(
  "Aged_niche" = list(
    "sender" = c("Aged_T_Cells"),
    "receiver" = c("Aged_B_Cells")),
  "Young_niche" = list(
    "sender" = c("Young_T_Cells"),
    "receiver" = c("Young_B_Cells"))
) # user adaptation required on own dataset

# 2. Calculate differential expression between the niches
assay_oi = "SCT"

DE_sender = calculate_niche_de(seurat_obj = TB_cell.combined %>% subset(features = lr_network$ligand %>% unique()), niches = niches, type = "sender", assay_oi = assay_oi) # only ligands important for sender cell types
DE_receiver = calculate_niche_de(seurat_obj = TB_cell.combined %>% subset(features = lr_network$receptor %>% unique()), niches = niches, type = "receiver", assay_oi = assay_oi) # only receptors now, later on: DE analysis to find targets
...
DE_receiver_targets = calculate_niche_de_targets(seurat_obj = TB_cell.combined, niches = niches, lfc_cutoff = lfc_cutoff, expression_pct = expression_pct, assay_oi = assay_oi)
...
dotplot = suppressWarnings(Seurat::DotPlot(TB_cell.combined %>% subset(idents = niches %>% unlist() %>% unique()), features = features_oi, assay = assay_oi))
...
lfc_plot
ggsave("T2B_lfc_plot.pdf", width = 8, height = 16)
...
ggsave("T2B_exprs_activity_target_plot.pdf", width = 20, height = 10)
...
ggsave("T2B_circos_output.pdf", width = 10, height = 10)
